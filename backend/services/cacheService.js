...
class CacheService {
  ...
  static async initialize() {
    try {
      ...
      if (this.isClusterMode) {
        this.client = await this.initializeCluster(config);
      } else {
        this.client = await this.initializeStandalone(config);
      }
  ...
  static async initializeCluster(config) {
    ...
    const client = redis.createCluster(clusterConfig);
  ...
  // 🎯 ENHANCED GET WITH COMPRESSION & FALLBACK
  ...
  // 🎯 ENHANCED SET WITH COMPRESSION & OPTIONS
  static async set(key, value, options = {}) {
    ...
    try {
      ...
      const expiration = options.expiration || 3600; // Default 1 hour
      const setOptions = {};
    ...
    try {
      ...
      for (const [key, value] of keyValuePairs) {
        ...
        if (options.expiration) {
          setOptions.push('EX', options.expiration);
        }
  ...
  static async checkRateLimit(key, limit, windowMs) {
    ...
    const result = await this.client.zRemRangeByScore(key, 0, windowStart);
